...
if [ "$RAPD_HOME" != "" ]; then
  ...
  # X-ray Analysis
  echo "#! /bin/bash" > $RAPD_HOME/bin/rapd.analysis
  echo "$SAFE_PREFIX\/bin\/rapd.python $SAFE_PREFIX\/src\/plugins\/analysis\/commandline.py \"\$@\"" >>$RAPD_HOME/bin/rapd.analysis
  chmod +x $RAPD_HOME/bin/rapd.analysis
