<?php 
  ...
  if (mysql_num_rows($result) > 0) {
    while ($sql = mysql_fetch_object($result)) {
      ...
      {
        $sql2    = "SELECT integrate_status,type,repr,timestamp,TIMESTAMPDIFF(SECOND,timestamp,NOW()) AS result_age FROM integrate_results WHERE integrate_result_id='$id'";
        $result2 = @mysql_query($sql2,$connection) or die(mysql_error());
        $sql2    = mysql_fetch_object($result2);
      ...
      elseif ($type == 'merge')
      {
        $sql2    = "SELECT merge_status,repr,timestamp,TIMESTAMPDIFF(SECOND,timestamp,NOW()) AS result_age FROM merge_results WHERE merge_result_id='$id'";
        $result2 = @mysql_query($sql2,$connection) or die(mysql_error());
        $sql2    = mysql_fetch_object($result2);
