...
if ($_POST[del_user] != "")
  {
	...
	echo "<P>User $_POST[del_user] has been trashed from the database.</P>";
      }else{
      echo "<P>User $_POST[del_user] could not be located in the database.</P>";
      }
	
    //delete the user from the banned list
    $del_banned = "DELETE FROM banned WHERE no_access = '$_POST[del_user]'";
    $result = @mysql_query($del_banned,$connection) or die(mysql_error());

    //now remove the trips
    $sql_trips    =  "SELECT * FROM trips WHERE username = '$_POST[del_user]'";
    $result_trips =  @mysql_query($sql_trips, $connection) or die(mysql_error());   
    $num_trips    =  mysql_num_rows($result_trips);
    if ($num_trips != 0)
      {
	while ($my_sql_trips = mysql_fetch_object($result_trips))
	  {
	    $my_drd  = $my_sql_trips -> data_root_dir;
	    remove_trip($_POST[del_user],$my_drd);
	  }      
      }else{
        echo "<p>User $_POST[del_user] had no trips to be deleted.</P>";
      }     
  }
