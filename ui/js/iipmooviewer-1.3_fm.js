...
var IIP = new Class({
  ...
  loadGrid: function(){
   ...
   var endx =  Math.ceil( ((len + this.rgn_x)/this.tileSize[0]) - 1 );

   len = this.rgn_h;
   if( this.hei < this.rgn_h ) len = this.hei;
   var endy = Math.ceil( ( (len + this.rgn_y)/this.tileSize[1]) - 1 );
...
//  var endy = Math.floor( (len + this.rgn_y) / this.tileSize[1] );
  ...
  createWindows: function(){
    ...
    if( Browser.Engine.trident5 ){
      document.id('info').setStyle( 'filter', 'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true,src="./css/images/iip.32x32.png",sizingMethod=scale)' );
    }
  ...
  createNavigationWindow: function() {
	...
	events: {
	    dblclick: function(){document.id('navbuttons').slide('toggle');}
	}
    });
   ...
   var loadBarContainer = new Element('div', {
    ...
    document.id('reset').addEvent( 'click', function(){ window.location=window.location; }  );
    document.id('shiftLeft').addEvent( 'click', function(){ this.scrollTo(-this.rgn_w/3,0); }.bind(this) );
    document.id('shiftUp').addEvent( 'click', function(){ this.scrollTo(0,-this.rgn_h/3); }.bind(this) );
    document.id('shiftDown').addEvent( 'click', function(){ this.scrollTo(0,this.rgn_h/3); }.bind(this) );
    document.id('shiftRight').addEvent( 'click', function(){ this.scrollTo(this.rgn_w/3,0); }.bind(this) );
  ...
  refreshLoadBar: function() {
    ...
    if( this.nTilesLoaded == this.nTilesToLoad ){
      ...
      document.id('target').setStyle( 'cursor', 'move' );
...
var IIP_FM = new Class({
   ...
   /* Initialize some variables. The constructor takes 4 arguments:
	...
	ii) A hash containting:
	      ...
	      credit:  image copyright or information (optional)
              ...
              showNavButtons: whether to show navigation buttons: true (default) or false 
	      scale:   pixels per mm
	      fm_mode: mode of the viewer 1=one image no predictions, 2=one image with
	               predictions, 3=two images no predictions, 4=two images with predictions
	      fm_images_reprs: a list of image names to be displayed
   */
    ...
    initialize: function( main_id, options ) {
        ...
        if( $type(options['image']) == 'array' ){
  	    for( i=0; i<options['image'].length;i++ ){
            ...
            this.images[0] = { src:options['image'], sds:"0,90"}; 
        }

        this.credit = options['credit'] || null;
        this.scale = options['scale'] || null;

        // Specialized pass-ins
        this.fm_mode = options['fm_mode'] || null;
        this.fm_image_reprs = options['fm_image_reprs'] || null;
  ...
  zoomIn: function (){
	...
	this.rgn_y = this.hei/2 - (this.rgn_h/2);
      ...
      if (this.request == true) this.requestImages();
      this.positionZone();
      if( this.scale ) this.setScale();
  ...
  zoomOut: function(){
    ...
    createNavigationWindow: function() {
	    ...
	    events: {
	        dblclick: function(){
	            document.id('navbuttons').slide('toggle');
	            document.id('transslider').slide('toggle');
                    document.id('imageselect').slide('toggle');
	        }
	    }
        });
        toolbar.store( 'tip:text', '* Drag to move<br/>* Double Click to show/hide navigation buttons' );
        ...
        if (this.fm_mode)
        {
          switch(this.fm_mode)
          {   
      	    // Single image, no predictions
            case 1:
                // Load the images
                //iip.images[0] = {src: iip.images[0].src, sds:'100,90'};

                //Create the image label here - labelled as transslider as they are interchangeable
	        var transslider = new Element('div', {
			                  id: 'transslider',
			                  html: '<h2>'+this.fm_image_reprs[0]+'</h2><br/>'
			              });
                transslider.injectInside( navcontainer );
	        // Hide our slider buttons if requested
    		if( this.showNavButtons == false ) transslider.slide('out');
		transslider.set('slide', {duration: 300, transition: Fx.Transitions.Quad.easeInOut, mode:'vertical'});
                break;
            
            // Single image with predictions    
            case 2:
                // Load the second image
                iip.images[1] = {src: iip.images[0].src, sds:'100,90'};

	        //Create the transparency slider here
	        var transslider = new Element('div', {
	                                  id: 'transslider',
	                                  html: '<h2>'+this.fm_image_reprs[0]+'</h2><br/><h3>Move slider to adjust predictions</h3><br/><div id="trans-area"><div id="trans-knob"></div><span id="baselabel">Image</span><span id="overlaylabel">Preds</span><br/></div>'
			              });
	        transslider.injectInside( navcontainer );
	        var slider = new Slider( document.id('trans-area'), document.id('trans-knob'), { range: [0,100],
	                             onChange: function(pos){
	    		                           this.opacity = pos/100.0;
			                           $$('#target img.layer1').set( 'opacity', this.opacity );
			                       }.bind(iip)
			         });
	        document.id('baselabel').setStyle('left', 25);
	        document.id('overlaylabel').setStyle('left', 200);
	        // Hide our slider buttons if requested
    	        if( this.showNavButtons == false ) transslider.slide('out');
	        transslider.set('slide', {duration: 300, transition: Fx.Transitions.Quad.easeInOut, mode:'vertical'});
	        break;

            // Pair of images without overlays
            case 3:
                //Load the image
                //iip.images[0] = {src: iip.images[0].src, sds:'100,90'};

                // Create the radio buttons for image selection here
                var imageselect = new Element('div',{
                                          id: 'imageselect',
                                          //style: 'text-align:left;width:100%;margin-left:2em;margin-top:0.5em;padding:0px',
                                          html: '<h2>Select image to view</h2><br/><p class="radio" style="margin-top:5px"><input type="radio" name="" id="first-image" value="000" checked="checked"/>&nbsp;<span id="first-image-label">'+this.fm_image_reprs[0]+'</span></p><p class="radio" style="margin-top:5px"><input type="radio" name="" id="second-image" value="200"/>&nbsp;<span id="second-image-label">'+this.fm_image_reprs[1]+'</span></p>'
                                      });
                imageselect.injectInside( navcontainer );
                // Hide our image selector buttons if requested
                if( this.showNavButtons == false ) imageselect.slide('out');
                imageselect.set('slide', {duration: 300, transition: Fx.Transitions.Quad.easeInOut, mode:'vertical'});

                // The first image is selected at load time
                document.id('first-image').getParent().setStyle('color','red');

                // First image radio button click
                document.id('first-image').addEvent('click', function(){
                    $$('p.radio').setStyle('color','#aaa');
                    document.id('first-image').getParent().setStyle('color','red');
                    iip.images[0] = {src: iip.images[0].src, sds: '0,90'};
                    iip.requestImages();
                });

                // Second image radio button click
                document.id('second-image').addEvent('click', function(){
                    $$('p.radio').setStyle('color','#aaa');
                    document.id('second-image').getParent().setStyle('color','red');
                    iip.images[0] = {src: iip.images[0].src, sds: '200,90'};
                    iip.requestImages();
                });

                break;

            // Pair of images with overlays
            case 4:
                // Load the second image
                iip.images[1] = {src: iip.images[0].src, sds:'100,90'};
                ...
                document.id('second-image').addEvent('click', function(){
                    ...
                    document.id('second-image').getParent().setStyle('color','red');
                    iip.images[0] = {src: iip.images[0].src, sds: '200,90'};
                    iip.images[1] = {src: iip.images[0].src, sds: '300,90'};   
                    iip.requestImages();
                });
                ...
                var transslider = new Element('div', {
                                          id: 'transslider',
                                          html: '<h3>Move slider to adjust predictions</h3><br/><div id="trans-area"><div id="trans-knob"></div><span id="baselabel">Image</span><span id="overlaylabel">Preds</span><br/></div>'
                                      });
                transslider.injectInside( navcontainer );
                var slider = new Slider( document.id('trans-area'), document.id('trans-knob'), { range: [0,100],
                                     onChange: function(pos){
