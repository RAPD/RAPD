                    ...
                    onSelect: function(e, context){
                        if ($(context).attr('id') > 0)
                        {
                            switch($(this).attr('id'))
                            {
                                ...
                                case 'snap_header':
                                    //new-style modal dialog
                                    $dialog_generic.load("header.php",
                                        {result_id:$(context).attr('id'),
                                         start_repr:$(context).text()},
                                        function(response, status, xhr) {
                    ...
                    onSelect: function(e, context){
                        if ($(context).attr('id'))
                        {
                            switch($(this).attr('id'))
                            {
                                ...
                                case 'run_settings':
                                    tb_show("Setings for "+$(context).text(),"run_settings.php?height=300&width=550&result_id="+$(context).attr('id')+"&start_repr="+$(context).text()+"&type=reprocess&ip="+my_ip);
                                    break;

                                case 'run_header':
                                    tb_show("Header information for "+$(context).text(),"run_header.php?height=620&width=550&result_id="+$(context).attr('id')+"&start_repr="+$(context).text());
                                    break; 
...
function resultsRefresh(again) {
    ...
    $.getJSON("get_results.php?datadir="+my_datadir+"&id="+lastresultID+"&timestamp="+lastresultTime, function(json) {
        if(json.length)
        {
...
function rankingRefresh() {
    ...
    $.getJSON("get_rankings.php?datadir="+my_datadir+"&id="+lastsnaprankID, function(json) {
        if(json.length > 1) {
    ...
    $.getJSON("get_run_rankings.php?datadir="+my_datadir+"&id="+lastrunrankID, function(json) {
        if(json.length > 1) {
...
function processesRefresh(again) {
    ...
    $.getJSON("get_processes.php?datadir="+my_datadir, function(json) {
        if(json.length)
        {
...
function statusMonitor() {
    $.getJSON("get_instance_info.php?datadir="+my_datadir+"&ip="+my_ip, function(json) {
        ...
        {
            $('#led_button').show();	
            $.getJSON("get_status.php?beamline="+my_beamline, function(json) {
                my_beamline    = json[0];
...
function settingsMonitor() {
  ...
  $.getJSON("get_settings.php?datadir="+my_datadir+"&id="+lastsettingID, function(json) {
    if(json.length > 1)
    {
      if(json[0]>0)
      {
...
function cloudMonitor() {
    ...
    $.getJSON("get_cloud.php?datadir="+my_datadir+"&user="+my_user+"&trip_id="+my_trip_id+"&ip_address="+my_ip+"&id="+lastcloudID, function(json) {
        if(json.length > 1)
        {
...
function loadSnap(my_res_id) {
  ...
  $.getJSON("get_result_data.php?res_id="+my_res_id+"&datadir="+my_datadir+"&trip_id="+my_trip_id, function(json) {
    if (json.length) {
      if (json[2] == 'None')
      {
        //Summary tab
...
//                DisplaySample(my_res_id);
            ...
            else if (json[6] in {Success:1,SUCCESS:1})  //== 'Success')
            {
				...
				if (currentanalysisID === 0) {
					$.getJSON("get_analysis_data.php?res_id="+my_res_id+"&datadir="+my_datadir+"&trip_id="+my_trip_id+"&username="+my_user, function(json) {
				        if (json.length) {
