    ...
    setTimeout(function() {
      ...
      // PDBQuery
        ...
        if (self.full_result.results.pdbquery) {
          ...
          const component_name = (plugin.type.toLowerCase().charAt(0).toUpperCase() + plugin.type.toLowerCase().slice(1) + plugin.id.toLowerCase() + plugin.version.replace(/\./g, '') + 'Component').toLowerCase();
