...
#        self.datasets = self.command[2]['MergeMany'] # RAPD 1 - This is a dict of all results.  Each result is a dict.
    ...
    def preprocess(self):
        ...
        for count, dataset in enumerate(self.datasets):
            ...
            if self.user_spacegroup != 0:
                self.logger.debug('HCMerge::Converting %s to %s and copying to Working Directory.' % str(dataset), str(self.user_spacegroup))
    ...
    def process(self):
        ...
        # combine the files with POINTLESS and scale with AIMLESS
...
#            pair.join()
    ...
    def postprocess(self):
        ...
        self.logger.debug('HCMerge::Making log file and cleaning up in postprocess.')
...
#        most_wedges = set(list(chain.from_iterable(most_wedges)))
    ...
    def merge_wedges(self, wedge_files):
        """
        Merge all the separate wedges together at the cutoff value. Separated from process so that I can use it in rerun.
    ...
    def cleanup(self):
        ...
        killlist = []
        killext = ['_pointless.sh', '_pointless.mtz', '_pointless.log']
        for ext in killext:
            for prefix in self.id_list:
                killlist.append(prefix+ext)
#        for itm in self.merged_files:
#            killlist.append(itm+'_aimless.sh')
        filelist = [ f for f in os.listdir(self.settings['work_directory']) if f.endswith(".sh") or f.endswith(".log")
                     or f.endswith(".mtz")]
...
#        purgelist = [f for f in filelist if f not in safelist]
    ...
    def get_dicts(self, file):
        ...
        self.data_files = tmp['data_files']
        self.id_list = tmp['id_list']
        self.results = tmp['results']
        self.graphs = tmp['graphs']
        self.matrix = tmp['matrix']
        self.merged_files = tmp['merged_files']
