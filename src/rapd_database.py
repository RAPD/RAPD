...
class Database:
    ...
    def __init__(self,settings,logger):
        ...
        logger.info( 'Database::__init__')
        
        #store passed in variables
        self.db_host       = settings['db_host']
        self.db_user       = settings['db_user']
        self.db_data_name  = settings['db_data_name']
        self.db_users_name = settings['db_users_name']
        self.db_cloud_name = settings['db_cloud_name']
        self.db_password   = self.Decode(settings['db_password'])
        self.logger        = logger
    ...
    def connectToData(self):
        ...
        while (attempts < mysql_attempts):
            try:
                #connect
                connection = MySQLdb.connect( host = self.db_host,
    ...
    def addImage(self,data):
        ...
        self.logger.debug('Database::addImage')
        self.logger.debug(data)
        connection,cursor = self.connectToData()
        ...
        try:
            cursor.execute("""INSERT INTO images (fullname,
                                                  ...
                                                  beam_size_y,
                                                  gauss_x,
                                                  gauss_y) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                                                                   (data['fullname'],
                                                                    data['adsc_number'],
                                                                    data['adc'],
                                                                    data['axis'],
            ...
            if errno == 1062:
         	...
         	return(data,False)
                ...
                self.logger.exception('ERROR : unknown IntegrityError exception in Database::addImage')
                self.logger.warning(errno)
                self.logger.warning(strerror)
            return(False)
        
        except:
            self.logger.exception('ERROR : unknown exception in Database::addImage')
            self.closeConnection(connection,cursor)
            return(False)
    ...
    def updateCurrent(self,in_dict):
        ...
        try:
            ...
            if in_dict.has_key('setting_id'):
                self.logger.debug('Updating current table using setting_id')
            ...
            if in_dict.has_key('data_root_dir'):
                self.logger.debug('Updating current table using data_root_dir')
            ...
            if in_dict.has_key('puckset_id'):
                self.logger.debug('Updating current table using puckset_id')
                if (do_and):
                    command += ','
    ...
    def getSettingsByBDT(self,beamline,data_root_dir='DEFAULTS',setting_type='GLOBAL'):
        ...
        self.logger.debug('Database::getSettingsByBDT beamline:%s drd:%s type:%s' % (beamline,data_root_dir,setting_type))
    ...
    def makeNewResult(self,rtype,process_id=0,data_root_dir=None):
        ...
        self.logger.debug('Database::makeNewResult  rtype:%s' % rtype,process_id)
    ...
    def addIntegrateResult(self,dirs,info,settings,results):
        ...
        #Catch non-numeric entries and set to 0
        if (results['summary']['CC_cut'] == False):
            results['summary']['CC_cut'] = [0.0,0.0]
        if (results['summary']['RCR_cut'] == False):
            results['summary']['RCR_cut'] = [0.0,0.0]
            ...
            try:
                if (results['status'] in ('SUCCESS','WORKING','FAILED')):    
                    ...
                    tmp_insert_values = [ results['summary']['scale_spacegroup'],
                                          ...
                                          cell[4],
                                          cell[5],
                                          results['summary']['CC_anom_overall'],
                                          results['summary']['CC_cut'][0],
                                          results['summary']['CC_cut'][1],
                                          results['summary']['RCR_anom_overall'],
                                          results['summary']['RCR_cut'][0],
                                          results['summary']['RCR_cut'][1] ]
    ...
    def addSadResult(self,dirs,info,settings,results):
        ...
        try:
            ...
            #Autosol section
            try:
                if(results['AutoSol results'] != 'None'):
                    #If this is an update to a run
                    if (sad_result_dict):
    ...
    def addShelxdResult(self,shelx_results,sad_result_id,shelxd_result_id=None):
        ...
        try:
            ...
            #regardless of failure level
            command = '''UPDATE shelxd_results SET sad_result_id=%s,
                                                best_occ=%s,
                                                trials=%s,
                                                cca=%s,
                                                cca_max=%s,
                                                ...
                                                fom_mean=%s,
                                                fom_stddev=%s,
                                                timestamp=NOW() WHERE shelxd_result_id=%s'''
            ...
            insert_values = [sad_result_id,
                             ...
                             shelx_results['shelxd_best_cc'],
                             cca_stats[0],
                             cca_stats[1],
                             cca_stats[2],
                             cca_stats[3],
                             shelx_results['shelxd_best_ccw'],
                             ccw_stats[0],
                             ccw_stats[1],
                             ccw_stats[2],
                             ccw_stats[3],
                             shelx_results['shelxd_fom'][int(shelx_results['shelxd_best_try'])-1],
                             fom_stats[0],
                             fom_stats[1],
                             fom_stats[2],
                             fom_stats[3],
    ...
    def addShelxeResult(self,shelx_results,sad_result_id,shelxe_result_id=None):
        ...
        try:
            ...
            insert_values = [sad_result_id,
                             solution,
                             json.dumps(shelx_results['shelxc_res'][:len(shelx_results['shelxc_res'])/2]).replace(' ',''),
                             len(shelx_results['shelxe_sites']),
                             shelx_results['shelxe_inv_sites'],
                             shelx_results['shelxe_cc'][0],
                             shelx_results['shelxe_cc'][1],
                             shelx_results['shelxe_contrast'][(len(shelx_results['shelxe_contrast'])/2)-1],
                             shelx_results['shelxe_contrast'][-1],
                             shelx_results['shelxe_con'][(len(shelx_results['shelxe_con'])/2)-1],
                             shelx_results['shelxe_con'][-1],
                             shelx_results['shelxe_mapcc'][(len(shelx_results['shelxe_mapcc'])/2)-1],
    ...
    def addStatsResults(self,info,results):
        ...
        if result_id:
            ...
            try:
                if(results.has_key('Cell analysis results')):
                  if(not results['Cell analysis results'] in ('FAILED',None)):
                    self.addCellAnalysisResults(cell_analysis_results=results['Cell analysis results'],
                                                result_id=result_id)
            except:
    ...
    def getTypeByResultId(self,result_id):
        """
        ...
        query = "SELECT type FROM results WHERE result_id=%s"
    ...
    def getResultById(self,id,type):
        """
    ...
    def updateTrip(self,trip_id,date):
        ...
        #connect
        connection,cursor = self.connectToUser()
        ...
        try:
            ...
            if errno == 1062:
                ...
                if beamline == 'Z':
                    ...
                    cursor.execute("""UPDATE runs SET directory    = %s,
                                                           image_prefix = %s,
                                                           run_number   = %s,
                                                           start        = %s,
                                                           total        = %s,
    ...
    def inRun(self,image_id):
        ...
        if image_dict:
            ...
            query2 = 'select run_id from runs where directory=%s AND image_prefix=%s AND run_number=%s AND %s>=start AND %s<= start+total'
    ...
    def markMinikappaRequest(self,minikappa_id,mark):
        ...
        connection,cursor = self.connectToCloud()
    ...
    ##################################################################################################################
    def updateControllerStatus(self,controller_ip=None,data_root_dir=None,beamline=None,dataserver_ip=None,cluster_ip=None):
        """
        Log the controller as an update to the status_controller table in rapd_data
        """
        #self.logger.debug('Database::updateControllerStatus')
