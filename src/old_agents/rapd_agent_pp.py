...
class LabelitPP(Process,Communicate):
  ...
  def preprocessLabelit(self):
    ...
    """
    if self.verbose:
        self.logger.debug('LabelitPP::preprocessLabelit')
    import glob
    try:
      ...
      self.logger.exception('**ERROR in LabelitPP.preprocessLabelit**')
  ...
  def processLabelitPP(self):
    """
    Run labelit.precession_photo on dataset.
    """
    if self.verbose:
      self.logger.debug('LabelitPP::processLabelitPP')
    try:
      ...
      self.logger.exception('**Error in LabelitPP.processLabelitPP**')
