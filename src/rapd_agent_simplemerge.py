__author__ = "Surajit Banerjee, Frank Murphy"
...
"""
rapd_agent_simplemerge.py is a pipeline to be used by the rapd_cluster.py
                          process for combining two wedges created by
...
from rapd_agent_xia2 import Xia
from MySQLdb import TIME
import time
import unittest
from rapd_communicate import Communicate

class SimpleMerge(Xia,Communicate):
    """
...
class TestPipeline(unittest.TestCase):
    ...
    def test_pipeline(self):
        S = SimpleMerge(['SMERGE',{'work':'./'},{'original':{'merge_file':'../test_data/1_mergable.mtz'},'secondary':{'merge_file':'../test_data/2_mergable.mtz'},'repr':'test'},None],self.logger)
        S.join()
